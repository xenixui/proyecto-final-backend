...
const refKey = (brand, reference) =>
    `${normalize(brand).toLowerCase()}::${normalize(reference).toLowerCase()}`;
...
function saveProgress(progress) {
    ...
    progress.stats.totalProcessed =
        progress.stats.totalSuccess + progress.stats.totalFailed;
...
async function getBrandNames(connection, progress) {
    const [rows] = await connection.query(
        'SELECT name FROM brands ORDER BY id ASC',
    );
    const names = [
...
function addToProgress(progress, watch) {
    ...
    const existing = new Set(
        progress.completedModels.map((model) =>
            refKey(model.brandName, model.reference),
        ),
    );
...
function rememberFailure(progress, brandName, error) {
    ...
    progress.failedModels = [
        ...progress.failedModels.filter(
            (item) => normalize(item.brandName) !== brandName,
        ),
        failed,
    ];
}
...
async function collectTenReferences(progress, connection) {
    ...
    for (const brandName of brandNames) {
        ...
        try {
            const references = await apiGet('/reference/list', {
                brand: brandName,
            });
            ...
            for (const reference of references) {
                ...
                for (const watch of watches) {
                    ...
                    if (addToProgress(progress, watch)) {
                        added += 1;
                        knownRefs.add(
                            refKey(watch.brand, watch.reference_number),
                        );
                        console.log(
                            `JSON + ${watch.brand} | ${watch.model} | ${watch.reference_number}`,
                        );
                    }
                }
